...
- **`preview.html`** ― ★まずこれを開く。上の `.dc.html` を **外部ランタイム無しでそのままブラウザ表示できる形**に変換した静的プレビュー（Codex の UI 評価用）。
  レイアウト・配色・文言・SVG は `.dc.html` と同一。`<sc-if>` を `data-when` 属性＋数十行の素の JS に置換しただけで、`support.js` も `_ds/` も読み込まない。
  右上のコントロールで `normal` / `loading` / `empty` / `error` / 成功表示 を切り替えて各状態を確認できる（分岐ロジックは `.dc.html` の `renderVals()` と等価）。
  ※これは確認用。実装の視覚仕様として正なのは下の `.dc.html`。
